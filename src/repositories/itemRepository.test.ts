...
const ITEM_ROW = { id: ITEM_ID, name: "Standard Unit", price: 15000, weight_kg: 0.365 };
...
describe("getItem", () => {
  it("retrieves an item by id, mapping snake_case columns to the domain Item shape", async () => {
    ...
    expect(query.calledWith("SELECT id, name, price, weight_kg FROM items WHERE id = $1", [ITEM_ID])).to.equal(
      true
    );
    expect(item).to.deep.equal({ id: ITEM_ID, name: "Standard Unit", price: 15000, weightKg: 0.365 });
  });
...
describe("getAllItems", () => {
  it("retrieves and maps every item row", async () => {
    ...
    expect(query.calledWith("SELECT id, name, price, weight_kg FROM items ORDER BY id")).to.equal(true);
    expect(items).to.have.lengthOf(1);
    expect(items[0]).to.deep.equal({ id: ITEM_ID, name: "Standard Unit", price: 15000, weightKg: 0.365 });
  });
});
...
describe("createItem", () => {
  it("inserts a new item and returns the row it gets back, mapped to the domain shape", async () => {
    ...
    const item = await itemRepository.createItem(
      { name: "Deluxe Unit", price: toMoney(25000), weightKg: 1.2 },
      fakeExecutor(query)
    );
    ...
    expect(item).to.deep.equal({ id: insertedRow.id, name: "Deluxe Unit", price: 25000, weightKg: 1.2 });
  });
});
