...
describe("POST /v1/items", () => {
  it("creates an item and returns 201 with the generated id", async () => {
    const response = await request(app.callback())
      .post("/v1/items")
      .send({ name: "Deluxe Unit", price: 25000, weightKg: 1.2 });
  ...
  it("400 INVALID_ITEM_NAME for a missing/empty name", async () => {
    const response = await request(app.callback())
      .post("/v1/items")
      .send({ name: "", price: 25000, weightKg: 1.2 });
  ...
  it("400 INVALID_PRICE for a non-positive price", async () => {
    const response = await request(app.callback())
      .post("/v1/items")
      .send({ name: "Deluxe Unit", price: 0, weightKg: 1.2 });
  ...
  it("400 INVALID_WEIGHT_KG for a non-positive weightKg", async () => {
    const response = await request(app.callback())
      .post("/v1/items")
      .send({ name: "Deluxe Unit", price: 25000, weightKg: -1 });
...
describe("GET /v1/items", () => {
  it("returns the seeded item when nothing else has been created", async () => {
    ...
    expect(response.body).toEqual([{ id: itemId, name: "Standard Unit", price: 15000, weightKg: 0.365 }]);
  });
  ...
  it("includes newly created items", async () => {
    ...
    expect(response.body).toEqual(
      expect.arrayContaining([
        { id: itemId, name: "Standard Unit", price: 15000, weightKg: 0.365 },
        createResponse.body,
      ])
    );
  });
